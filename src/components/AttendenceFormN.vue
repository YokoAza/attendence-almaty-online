...
import InfoCommentN from "@/components/modal/InfoCommentN";
...
export default {
  ...
  computed: {
    ...
    commentsN() {
      var commentsN = [];
      this.$store.state.commentsN.map(
        (el) => (commentsN = commentsN.concat(el.comments))
      );
      return commentsN;
    },
    srezLevel() {
      ...
      if (this.valid) {
        if (!this.click) {
          ...
          if (this.currentGroup.change && !this.equal) {
            ...
            this.overlay = true;
            var response = await this.$store.dispatch("SetAttendenceN", {
              group: this.currentGroup,
